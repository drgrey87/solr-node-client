...
function createClient(host, port, core, path, agent, secure, bigint, ipVersion){
...
function Client(options){
   ...
   this.STREAM_HANDLER = 'stream?expr=';
...
/**
 * Streaming expression for matching query https://lucene.apache.org/solr/guide/6_6/streaming-expressions.html
 *
 * @param {Query|Object|String} query
 * @param {Function} callback(err,obj) - a function executed when the Solr server responds or an error occurs
 * @param {Error} callback().err
...
Client.prototype.stream = function(query,callback){
   return this.get(this.STREAM_HANDLER, query, callback);
}
...
Client.prototype.get = function(handler,query,callback){
   ...
   } else if (handler === this.STREAM_HANDLER) {
      pathArray = [this.options.path,this.options.core,handler + parameters];
   } else {
